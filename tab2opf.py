...
# -*- coding: utf-8 -*-
#
# extended and customized by tu-br
#
...
OPFTEMPLATEHEAD2 = """
        ...
        <DictionaryOutLanguage>en-uk</DictionaryOutLanguage>
    </x-metadata>
</metadata>
...
""")
    ...
    # dt, dd = r.split('\t', 1)
    head, the_rest = r.split('\t', 1)
    if '{' in head:
        tag = head.split('{')[1].split('}')[0]
    else:
        tag = ''
    # get rid of grammar info
    head = head.split(' {')[0]

    # these are useless for kindle
    if ' ' in head:
        continue
    derivatives, definition = the_rest.rsplit('\t', 1)
    derivatives = derivatives.split('\t')
    # http://spanish.about.com/od/nouns/a/making_plurals.htm
    if tag in ['f', 'm', 'mf', 'n']:
        if head[-1:] in 'aeiouéü':
            derivatives.append(head + 's')
        if head[-1:] in 'bcdfghjklmnñpqrstvwxyáíóú':
            derivatives.append(head + 'es')
    if len(derivatives) > 1:
        derivatives_str = '\n                <idx:infl>\n'
        for derivative in derivatives:
            if len(derivative) > 0:
                derivatives_str += '                    <idx:iform value="%s" />\n' % derivative
        derivatives_str += '                </idx:infl>\n'
    else:
        derivatives_str = ''
    if not UTFINDEX:
        head = normalizeUnicode(head, 'cp1252')
        definition = normalizeUnicode(definition, 'cp1252')
    dtstrip = normalizeUnicode(head)
...
"""    <idx:entry name="word" scriptable="yes">
        ...
        %s</idx:entry>
      <mbp:pagebreak/>
""" % (head, derivatives_str, dtstrip, definition))
    # print head
    i += 1
